...
namespace OURGL
{
    ...
    {
        mat<4, 4, float> viewPort = mat<4, 4, float>::identity();
        viewPort[0][0] = static_cast<float>(width) * 0.5f;
        viewPort[0][3] = static_cast<float>(width) * 0.5f;
        viewPort[1][1] = -static_cast<float>(height) * 0.5f;
        viewPort[1][3] = static_cast<float>(height) * 0.5f;
        return viewPort;
    }
        ...
        float* zBuffer)
    {
        ...
        mat<4, 4, float> viewProjection = getPerspective(90.0f, static_cast<float>(image.width()) / static_cast<float>(image.height()), 0.1f, 100.0f);
        viewProjection = viewProjection * getViewMatrix({ 0.f,0.f,2.f }, { 0.0f, 0.0f, -1.0f });
        viewProjection = getViewPortMatrix(image.width(), image.height()) * viewProjection;
        for (int face = 0; face < obj.nfaces(); face++)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                veci3 screen_coord;
                screen_coord.x = homo_point_pos[0];
                screen_coord.y = homo_point_pos[1];
